...
def get_log_sprite(is_left):
    log_sprites = [{"file_name": "log.png", "size": (140, 100)}]
    sprite = choice(log_sprites)
    print(sprite['file_name'])
    mob_sprite = pygame.image.load(f"images_src/{sprite['file_name']}").convert_alpha()
    mob_sprite = pygame.transform.scale(mob_sprite, (sprite['size']))
    mob_sprite = pygame.transform.flip(mob_sprite, is_left, False)
    return mob_sprite, sprite['size']
